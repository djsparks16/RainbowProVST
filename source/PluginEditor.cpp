...
void ScopeDisplay::paint(Graphics& g)
{
    ...
    auto mainView = hero.removeFromTop((int) (hero.getHeight() * 0.70f));
    auto footer = hero.reduced(0.0f, 2.0f);
    auto specArea = footer.removeFromLeft((int) (footer.getWidth() * 0.62f));
    ...
    for (int i = 0; i < 36; ++i)
    {
        ...
        auto c = Colour::interpolatedBetween(displayA(), displayB(), t).withAlpha(0.65f);
        g.setColour(c);
    ...
    : AudioProcessorEditor(&p), audioProcessor(p), scope(p), spectrum(p), lfoShape(p), matrixView(p)
{
    ...
    logo.setText("5parks VST3 · LEVEL 9000 BEEFCAKE CORE", dontSendNotification);
    logo.setColour(Label::textColourId, textHi());
    logo.setFont(FontOptions(28.0f, Font::bold));
    subtitle.setText("flagship bass synth · dark premium shell · giant hero display · reese · womp · screech · warhorn · human to hostile", dontSendNotification);
...
void FiveParksVST3AudioProcessorEditor::paint(Graphics& g)
{
    ...
    for (int i = 0; i < whiteKeys; ++i)
    {
        ...
        auto accentKey = (i % 2 == 0 ? displayA() : displayB()).withAlpha(0.18f);
        g.setColour(accentKey);
        g.fillRoundedRectangle(key.removeFromBottom(5.0f), 3.0f);
    }
}
...
void FiveParksVST3AudioProcessorEditor::resized()
{
    ...
    auto hero = area.removeFromTop(350);
    displayBox.setBounds(hero);

    area.removeFromTop(10);
    auto row1 = area.removeFromTop(240);
    ...
    area.removeFromTop(10);
    auto row2 = area;
    bassBox.setBounds(row2.removeFromLeft(720));
